...
class tareaController {
    async create(req: AuthRequest, res: Response): Promise<void> {
        try {
            ...
            const usuario = await getRepository(usuarioModel).findOneBy({ id: usuario_id });
    ...
    async update(req: AuthRequest, res: Response): Promise<void> {
        try {
            ...
            if (!tarea) {
                res.status(404).json({ error: "Tarea no encontrada o no tienes permiso para editarla." });
                return;
            }
            ...
            res.json(new responseUtils(true, [], resultado, "Tarea modificada correctamente"));
        } catch (error) {
            console.error("Error al actualizar la tarea:", error);
            res.status(500).json(new responseUtils(false, [], null, "Error en el servidor"));
        }
    }
    ...
    async get(Request: Request, Response: Response): Promise<void> {
        try {
            ...
            if (isNaN(id)) {
                await Response.status(400).json(new responseUtils(false, [], null, "ID inválido"));
                return
            }

            const producto = await getRepository(tareasModel).findOne({ where: { id } });
