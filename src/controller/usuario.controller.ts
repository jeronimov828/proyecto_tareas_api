...
class usuarioController {
  async create(req: Request, res: Response): Promise<void> {
    try {
      ...
      const verificarUsuario = await getRepository(usuarioModel).findOneBy({
        nombre,
      });
      if (verificarUsuario) {
        await res
          .status(400)
          .json(new responseUtils(false, [], null, "El usuario ya existe"));
        return;
      }
      ...
      const guardarNuevoUsuario =
        getRepository(usuarioModel).save(nuevoUsuario);
      await res.json(
        new responseUtils(
          true,
          [],
  ...
  async login(req: Request, res: Response): Promise<void> {
    ...
    try {
      const buscaUsuario = await getRepository(usuarioModel).findOneBy({
        nombre,
      });
      if (!buscaUsuario) {
        await res.json(
          new responseUtils(false, [], null, "El usuario no existe")
        );
        return;
      }
      ...
      const validarContrasena = await bcrypt.compare(
        contrasena,
        String(buscaUsuario.contrasena)
      );
      console.log(validarContrasena);
      if (!validarContrasena) {
        await res.json(
          new responseUtils(false, [], null, "Contraseña incorrecta")
        );
        return;
      }
      ...
      await res.json(
        new responseUtils(false, [], null, "Error en el servidor")
      );
      return;
    }
  }
